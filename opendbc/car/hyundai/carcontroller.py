...
class CarController(CarControllerBase, EsccCarController, LeadDataCarController, LongitudinalController, MadsCarController,
  ...
  def __init__(self, dbc_names, CP, CP_SP):
    ...
    self.btn_copies = 10 if CP.flags & HyundaiFlags.CANFD_ALT_BUTTONS else 20
      ...
      if (self.frame - self.last_button_frame) * DT_CTRL > 0.25:
        ...
        if CC.cruiseControl.cancel:
          ...
          if self.CP.flags & HyundaiFlags.CANFD_ALT_BUTTONS:
            ...
            for _ in range(self.btn_copies):
              can_sends.append(hyundaicanfd.create_buttons(self.packer, self.CP, self.CAN, CS.buttons_counter + 1, Buttons.CANCEL, CS.is_metric))
            self.last_button_frame = self.frame

        # cruise standstill resume
        elif CC.cruiseControl.resume:
          for _ in range(self.btn_copies):
            can_sends.append(hyundaicanfd.create_buttons(self.packer, self.CP, self.CAN, CS.buttons_counter + 1, Buttons.RES_ACCEL, CS.is_metric))
          self.last_button_frame = self.frame
